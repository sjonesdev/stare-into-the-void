...
exports.downloadProxy = functions.https.onRequest((req, res) => {
  functions.logger.log("Download proxy call");
  handleCors(req, res, () => {
    const url = req.body.data.url;
    functions.logger.log(`Got request to download for URL ${url}`);
    if (url) {
      https
        .get(url?.toString(), (resp) => {
          functions.logger.log(`Downloading for URL ${url}`);
          functions.logger.log(resp.headers);
          // todo validate request domain
          const chunks: Buffer[] = [];
          resp.on("data", (chunk: Buffer) => {
            functions.logger.log(`chunk(${chunk.length}): `, chunk);
            chunks.push(chunk);
          });
          resp.on("end", () => {
            ...
            // new Blob(chunks, { type: resp.headers["content-type"] })
            //   .arrayBuffer()
            //   .then((buf) => {
            //     functions.logger.log(
            //       `Got image buffer of size ${buf.byteLength}`,
            //       buf
            //     );
            //     res.status(200).send({
            //       data: { buffer: buf, type: resp.headers["content-type"] },
            //     });
            //   });
          });
        })
